...
trait SparkSessionWrapper {
  ...
  def startSpark(hadoopConf: Option[Configuration]): SparkSession = {
    ...
    val sparkConf = hadoopConf.foldLeft(new SparkConf(false)) { (sparkConf, hadoopConf) =>
      hadoopConf.asScala.foldLeft(sparkConf)((sc, entry) => sc.set(entry.getKey, entry.getValue))
    }
