...
case class Table private(dfs: DFSWrapper, spark: SparkSession, table: String, location: String, schema: StructType, partitionColumns: Seq[String], format: DataFormat, options: Map[String, String]) {
  ...
  private def createWriter(loadMode: LoadMode): OutputWriter = {
    OutputWriter.newFileSystemWriter(location, format, partitionColumns, options + ("emptyValue" -> ""), loadMode)
  }
}
...
object Table {
  ...
  class TableBuilder(table: String, database: String, location: String, schema: StructType) {
    ...
    private var partitionColumns: Seq[String] = Seq()

    private val defaultDSVOptions: Map[String, String] = Map("delimiter" -> "|")
    private var options: Map[String, String] = Map()


    def withPartitions(partitionColumns: Seq[String]): TableBuilder = {
    ...
    private def buildTable(format: DataFormat, options: Map[String, String], dfs: DFSWrapper, spark: SparkSession, external: Boolean): Table = {
      ...
      new Table(dfs, spark, fullTableName, location, schema, partitionColumns, format, options)
    }
    ...
    private def createHiveTable(format: DataFormat, options: Map[String, String], spark: SparkSession, external: Boolean): Unit = {
      ...
      val partitionColumnFields = partitionColumns.map(fieldMap)
