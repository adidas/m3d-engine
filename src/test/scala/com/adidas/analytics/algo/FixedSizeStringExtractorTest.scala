...
import com.adidas.analytics.TestUtils.ExtendedDataFrame
...
class FixedSizeStringExtractorTest extends FeatureSpec with BaseAlgorithmTest {
  ...
  private def createTable(tableName: String, database: String, schema: StructType, partitionColumns: Seq[String]): Table = {
    val table = Table.newBuilder(tableName, database, fs.makeQualified(new Path(hdfsRootTestPath, tableName)).toString, schema)

    if (partitionColumns.nonEmpty) {
      table.withPartitions(partitionColumns).buildParquetTable(DFSWrapper(fs.getConf), spark, external = true)
    } else {
      table.buildParquetTable(DFSWrapper(fs.getConf), spark, external = true)
    }
  }

  private def prepare(testResourceDir: String, partitionColumns: Seq[String] = Seq.empty, initialData: Boolean = true): Unit = {
    ...
    // create tables
    sourceTable = createTable(sourceTableName, database, sourceSchema, partitionColumns)
    targetTable = createTable(targetTableName, database, targetSchema, partitionColumns)
