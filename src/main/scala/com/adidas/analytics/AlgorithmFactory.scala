...
object AlgorithmFactory {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try {
      createAlgorithmInstance(spark, appClassName, appParamFile).run()
    } finally {
      spark.close()
    }
  }
  ...
  /**
    ...
    */
  private def createAlgorithmInstance(spark: SparkSession, className: String, configLocation: String): JobRunner = {
    ...
    className match {
      ...
      case "PartitionFullMaterialization" => PartitionMaterialization.newFullMaterialization(spark, dfs, configLocation)
