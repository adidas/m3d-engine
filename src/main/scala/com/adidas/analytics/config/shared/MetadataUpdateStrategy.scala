...
trait MetadataUpdateStrategy extends ConfigurationContext {
  ...
  protected def getMetaDataUpdateStrategy(
      targetTable: String,
    ...
    configReader.getAsOption[String]("metadata_update_strategy") match {
      ...
      case Some("SparkRecoverPartitionsCustom") =>
        RecoverPartitionsCustom(targetTable, partitionColumns)
      case Some(invalidConfig) =>
        throw new Exception(s"Invalid metadata update strategy $invalidConfig")
