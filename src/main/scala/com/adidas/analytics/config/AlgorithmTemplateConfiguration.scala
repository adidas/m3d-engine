...
trait AlgorithmTemplateConfiguration extends ConfigurationContext with ReadOperation with SafeWriteOperation {
  ...
  /**
    * This trait contains has the responsibility to obtain the required configurations
    * for a given algorithm. In this template, it can be seen that values like source and target tables,
    * dates, and readers and writers, are obtained in this class by mixing ConfigurationContext, ReadOperation
    * and SafeWriteOperation.
    *
    * At the same time, AlgorithmTemplateConfiguration is mixed in the AlgorithmTemplate class, so it can
    * use the values from the provided configuration.
    *
  ...
  //  protected val sourceLocation: String = configReader.getAs[String]("source_location").trim
...
//    InputReader.newFileSystemReader(sourceLocation, DataFormat.ParquetFormat())
  ...
  override protected val writer: OutputWriter.AtomicWriter = {
    /**
      ...
      * -- AppendJoinPartitions: which appends the records to existing ones in the partition directory by a Full Outer Join
      * -- AppendUnionPartition: which appends the records to existing ones in the partition directory by a Union All
      */
...
object AlgorithmTemplateConfiguration {
  ...
  /**
    * A companion object can alternatively be used to add helper methods
    * In this case, there is a method to convert a date string to a specific
    * date value, because in this example, date could also contain a string
    * such as today and yesterday, as well as a pattern.
    */
