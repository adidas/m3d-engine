...
trait BaseIntegrationTest extends BaseAlgorithmTest {
  ...
  protected val headerDirPath: Path = new Path(hdfsRootTestPath, s"$sourceDatabase/$tableName/header")
  ...
  protected def uploadParameters(testResourceDir: String, whichParamsFile: String = paramsFileName, whichParamsPath: Path = paramsFileHdfsPath): Unit = {
    copyResourceFileToHdfs(s"$testResourceDir/$whichParamsFile", whichParamsPath)
  }

  protected def createTargetTable(testResourceDir: String, targetPartitions: Seq[String], targetSchema: StructType): Table = {
  ...
  protected def setupInitialState(targetTable: Table, localDataFile: String, dataReader: FileReader): Unit = {
  ...
  protected def createPartitionedTargetTable(targetPartitions: Seq[String], targetSchema: StructType, tableName: String): Table = {
    val targetTableLocation = fs.makeQualified(new Path(hdfsRootTestPath, targetDirPath)).toString
    Table.newBuilder(tableName, targetDatabase, targetTableLocation, targetSchema)
      .withPartitions(targetPartitions)
      .buildParquetTable(DFSWrapper(fs.getConf), spark, external = true)
