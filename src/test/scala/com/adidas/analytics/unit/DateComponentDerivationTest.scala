...
class DateComponentDerivationTest extends FunSuite with SparkSessionWrapper with Matchers with BeforeAndAfterAll {

  class DateComponentDerivationSubClass extends DateComponentDerivation {
    def validateWithDateComponents(sourceDateColumnName: String,
                                   sourceDateFormat: String,
                                   targetDateComponentColumnNames: Seq[String])(inputDf: DataFrame): DataFrame = {
      super.withDateComponents(sourceDateColumnName, sourceDateFormat, targetDateComponentColumnNames)(inputDf)
    }
