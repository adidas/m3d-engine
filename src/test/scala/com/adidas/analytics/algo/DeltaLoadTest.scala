...
//      createParquetFileFromDSVfileandWriteToHDSF(testResourceDir, "delta_data.psv", "delta_schema.json")
  ...
  private def setupEnvironment(partitionColumns: Seq[String], paramsFileName: String, testResourceDir: String): Unit = {
  ...
  private def createTableWithSchema(tableName: String, partitionColumns: Seq[String], testResourceDir: String, dsvFileName: Option[String], schemaFileName: String): Table = {
    val tableLocation = fs.makeQualified(new Path(hdfsRootTestPath, s"$testDatabase/$tableName"))
