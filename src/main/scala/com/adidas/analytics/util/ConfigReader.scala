...
class ConfigReader(jsonContent: String) extends Serializable {
  ...
  JSON.globalNumberParser = (in: String) =>
    if (in.contains(decimalSeparator)) in.toDouble else in.toInt

  private lazy val config = JSON.parseRaw(jsonContent) match {
    case Some(JSONObject(obj)) => obj
    case _ =>
      throw new IllegalArgumentException(s"Wrong format of the configuration file: $jsonContent")
  }
  ...
  def getAsSeq[T](propertyName: String): Seq[T] =
    config.get(propertyName) match {
      ...
      case _ =>
        throw new IllegalArgumentException(s"Unable to find configuration property $propertyName")
    }
  ...
  def contains(propertyName: String): Boolean = config.contains(propertyName)
}
