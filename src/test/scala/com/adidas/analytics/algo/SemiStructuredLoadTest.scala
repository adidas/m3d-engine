...
import com.adidas.analytics.util.DataFormat.{JSONFormat, ParquetFormat}
import com.adidas.analytics.util.{DFSWrapper, LoadMode, OutputWriter}
...
class SemiStructuredLoadTest extends FeatureSpec with BaseAlgorithmTest {
  ...
  feature("Data can be loaded from source to target with append mode") {
    scenario("SemiStructured Data can be loaded with append mode by creating partitions from full path") {
      ...
      val sourceDirFullPath: Path = new Path(hdfsRootTestPath, s"$sourceDatabase/$tableNameJson/data/year=2018/month=01/day=02/")
      val targetDirFullPath: Path = new Path(hdfsRootTestPath, s"$targetDatabase/$tableNameJson/data")
      val headerDirPathPartFromFullPath: Path = new Path(hdfsRootTestPath, s"$sourceDatabase/$tableNameJson/header")
      val targetDirPathPartFromFullPath: Path = new Path(hdfsRootTestPath, s"$targetDatabase/$tableNameJson")
      ...
      val headerPath20180102 = new Path(headerDirPathPartFromFullPath, "year=2018/month=1/day=2/header.json")
      ...
      val dataWriter = OutputWriter.newFileSystemWriter(targetDirFullPath.toString, dataFormat, Seq("year", "month", "day"))
      ...
      // validating result
      val expectedDataLocation = resolveResource(s"$testResourceDir/lake_data_post.txt", withProtocol = true)
    ...
    scenario("Nested SemiStructured Data can be loaded with append mode by creating partitions from full path") {
      val tableNameJson: String = "test_table_semistructured"
      val paramsFileModdedRegexHdfsPath: Path = new Path(hdfsRootTestPath, paramsFileName)
      val sourceDirFullPath: Path = new Path(hdfsRootTestPath, s"$sourceDatabase/$tableNameJson/data/year=2018/month=01/day=02/")
      val targetDirFullPath: Path = new Path(hdfsRootTestPath, s"$targetDatabase/$tableNameJson/data")
    ...
    scenario("SemiStructured Parquet Data can be loaded with append mode by creating partitions from full path") {
      val tableNameJson: String = "test_table_semistructured"
      val paramsFileModdedRegexHdfsPath: Path = new Path(hdfsRootTestPath, paramsFileName)
      val sourceDirFullPath: Path = new Path(hdfsRootTestPath, s"$sourceDatabase/$tableNameJson/data/year=2018/month=01/day=02/")
      val targetDirFullPath: Path = new Path(hdfsRootTestPath, s"$targetDatabase/$tableNameJson/data")
    ...
    scenario("SemiStructured Data can be loaded with append mode with evolving schema") {
      ...
      val sourceDirFullPath20180103: Path = new Path(hdfsRootTestPath, s"$sourceDatabase/$tableNameJson/data/year=2018/month=01/day=03/")
      prepareSourceData(testResourceDir, Seq("data-nodate-part-00002.txt"), sourceDirFullPath20180103)
      val targetSchemaEvolved = DataType.fromJson(getResourceAsText(s"$testResourceDir/target_schema_evolved.json")).asInstanceOf[StructType]
    ...
    scenario("SemiStructured Data can be loaded with append mode with dropping columns") {
      ...
      val headerPath20180102 = new Path(headerDirPathPartFromFullPath, "year=2018/month=1/day=2/header.json")
      val headerPath20180103 = new Path(headerDirPathPartFromFullPath, "year=2018/month=1/day=3/header.json")
      val targetPath20180102 = new Path(targetDirFullPath, "year=2018/month=1/day=2")
      ...
      // prepare new data
      val sourceDirFullPath20180103: Path = new Path(hdfsRootTestPath, s"$sourceDatabase/$tableNameJson/data/year=2018/month=01/day=03/")
      val sourceDirFullPath20180104: Path = new Path(hdfsRootTestPath, s"$sourceDatabase/$tableNameJson/data/year=2018/month=01/day=04/")
      prepareSourceData(testResourceDir, Seq("data-nodate-part-00002.txt"), sourceDirFullPath20180103)
      prepareSourceData(testResourceDir, Seq("data-nodate-part-00003.txt"), sourceDirFullPath20180104)
      val targetSchemaDroppedCol = DataType.fromJson(getResourceAsText(s"$testResourceDir/target_schema_column_dropped.json")).asInstanceOf[StructType]
    ...
    scenario("SemiStructured Data cannot be loaded when data contains more columns than target schema") {
      val tableNameJson: String = "test_table_semistructured"
      val paramsFileModdedRegexHdfsPath: Path = new Path(hdfsRootTestPath, paramsFileName)
      val sourceDirFullPath: Path = new Path(hdfsRootTestPath, s"$sourceDatabase/$tableNameJson/data/year=2018/month=01/day=02/")
      val targetDirFullPath: Path = new Path(hdfsRootTestPath, s"$targetDatabase/$tableNameJson/data")
      ...
      assert(caught.getMessage.equals(s"Schema does not match the input data for some of the input folders."))
    ...
    scenario("SemiStructured Data cannot be loaded with wrong configuration") {
      ...
      logger.info(caught.getMessage)
      assert(caught.getMessage.equals(s"Unsupported data type: unstructured in AppendLoad or the configuration file is malformed."))
    }

    scenario("Loading semistructured data when some header files are available and schemas are the same") {
      val tableNameJson: String = "test_table_semistructured"
      val paramsFileModdedRegexHdfsPath: Path = new Path(hdfsRootTestPath, paramsFileName)
      val sourceDirFullPath20180101: Path = new Path(hdfsRootTestPath, s"$sourceDatabase/$tableNameJson/data/year=2018/month=01/day=01/")
      val sourceDirFullPath20180102: Path = new Path(hdfsRootTestPath, s"$sourceDatabase/$tableNameJson/data/year=2018/month=01/day=02/")
      val targetDirFullPath: Path = new Path(hdfsRootTestPath, s"$targetDatabase/$tableNameJson/data")
      val headerDirPathPartFromFullPath: Path = new Path(hdfsRootTestPath, s"$sourceDatabase/$tableNameJson/header")
      val targetDirPathPartFromFullPath: Path = new Path(hdfsRootTestPath, s"$targetDatabase/$tableNameJson")
      ...
      prepareSourceData(testResourceDir, Seq("data-nodate-part-00001.txt"), sourceDirFullPath20180101)
      prepareSourceData(testResourceDir, Seq("data-nodate-part-00002.txt"), sourceDirFullPath20180102)
      uploadParameters(testResourceDir, paramsFileName, paramsFileModdedRegexHdfsPath)
      ...
      val actualSchema20180105 = DataType.fromJson(fs.readFile(headerPath20180102)).asInstanceOf[StructType]
  ...
  private def setupInitialState(localDataFile: String, dataReader: FileReader, dataWriter: OutputWriter): Unit = {
