...
class AppendLoadTest extends FeatureSpec with BaseAlgorithmTest {
  ...
  feature("Reader mode can be specified in configuration") {
    ...
    scenario("when reader mode is set to FAILFAST and there are malformed records, an exception is thrown") {
      val testResourceDir = "reader_mode_specification"
      val headerPath20180422 = new Path(headerDirPath, "year=2018/month=4/day=22/header.json")
      val targetPath20180422 = new Path(targetDirPath, "year=2018/month=4/day=22")
    ...
    scenario("when reader mode is set to PERMISSIVE and there are malformed records, data is incorrectly loaded") {
      val testResourceDir = "reader_mode_specification"
      val headerPath20180422 = new Path(headerDirPath, "year=2018/month=4/day=22/header.json")
      val targetPath20180422 = new Path(targetDirPath, "year=2018/month=4/day=22")
  ...
  private def createTargetTable(testResourceDir: String, targetPartitions: Seq[String], targetSchema: StructType): Table = {
